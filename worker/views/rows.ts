...
/* ---------------------------------------------------------------------------
   The board's row cards and goal furniture. Class lists are verbatim string
   literals throughout so
 ...
 */
export function goalRailHtml(value: number, target: number, compact = false): Html {
  ...
  return html`<span
    class="block overflow-hidden rounded-full bg-heat-0 ${scale}"
    aria-hidden="true"
    ><span
      ...
      style="width:${(fraction * 100).toFixed(2)}%;background-size:${size}% 100%"
    ></span
  ></span>`;
}
 ...
 */
export function boardGoalLineHtml(goal: BoardGoal): Html | null {
  if (goal.nextMilestone === null) return null;
  return html`<div class="mt-[1.15rem] max-w-[30rem]">
    <div
      class="flex flex-wrap items-baseline justify-between gap-x-5 gap-y-1 font-mono text-[0.74rem]"
    >
      <p class="text-dim">
        <span class="text-[0.6rem] tracking-[0.18em] text-dimmer uppercase"
          >next board target</span
        >
        <span class="tabular-nums text-ink">${formatNumber(goal.total)}</span> of
        <span class="tabular-nums">${formatNumber(goal.nextMilestone)}</span>
      </p>
      <p class="tabular-nums text-dimmer">${formatNumber(goal.remaining ?? 0)} to go</p>
    </div>
    <div class="mt-[0.6rem]">${goalRailHtml(goal.total, goal.nextMilestone)}</div>
  </div>`;
}
 ...
 */
export function userGoalsBandHtml(goals: UserGoals): Html {
  const parts: Html[] = [];
  ...
  if (goals.nextMilestone !== null) {
    ...
    parts.push(
      html`<div class="flex items-center gap-[0.6rem]">
        ...
        <dd class="flex items-center gap-[0.6rem] text-dim">
          <span
            ><strong class="font-medium tabular-nums text-ink"
              >${formatNumber(goals.toMilestone ?? 0)}</strong
            >
            to <span class="tabular-nums">${formatNumber(goals.nextMilestone)}</span></span
          >${goalRailHtml(reached, goals.nextMilestone, true)}
        </dd>
      </div>`,
    );
  }
  ...
  if (goals.above) {
    ...
    const gap =
      above.behind > 0
        ? html`<strong class="font-medium tabular-nums text-ink"
              >${formatNumber(above.behind)}</strong
            >
            behind <span class="text-ink">${above.login}</span>`
        : html`level with <span class="text-ink">${above.login}</span>`;
    parts.push(
      html`<div class="flex min-w-0 items-center gap-[0.6rem]">
        <dt class="${TERM}">rank gap</dt>
        <dd class="min-w-0 break-words text-dim">
          ${gap} <span class="text-dimmer">(${formatOrdinal(above.rank)})</span>
        </dd>
      </div>`,
    );
    ...
    parts.push(
      html`<div class="flex items-center gap-[0.6rem]">
        ...
        <dd class="text-dim">
          leads by
          <strong class="font-medium tabular-nums text-ink"
            >${formatNumber(goals.leadMargin)}</strong
          >
        </dd>
      </div>`,
    );
  ...
  return html`<div
    class="border-t border-line-soft bg-[rgba(10,12,24,0.32)] px-[1.3rem] py-[0.72rem] max-phone:px-4 max-phone:py-[0.65rem]"
  >
    <dl
      class="flex flex-wrap items-center gap-x-6 gap-y-[0.55rem] font-mono text-[0.72rem] leading-tight max-phone:gap-x-[1.1rem]"
    >
      ${parts}
    </dl>
  </div>`;
...
function userRowBody(options: UserRowOptions, withChevron: boolean): Html {
  ...
  const chevron = withChevron
    ? html`<svg
        class="block h-auto w-[9px] text-dimmer transition-[color,rotate] duration-200 group-hover:text-dim group-open:rotate-180"
        viewBox="0 0 10 6"
        fill="none"
        ><path
  ...
  const score = peak
    ? html`<span
        ...
        >peak ${formatNumber(peak.count)} on ${formatDayShort(peak.date)}${leadsPeak
          ? html`<span class="sr-only"> — highest single day on the board</span>`
          : null}</span
      >`
    : html`<span class="mt-[0.4rem] text-[0.68rem] text-dim">no activity</span>`;

  return html`<div
      class="flex flex-col items-start gap-[0.3rem] text-[0.78rem] tracking-[0.06em] [grid-area:rank] ${lead
        ? "text-accent"
        : "text-dimmer"}"
      aria-hidden="true"
    >
      <span>${formatRank(rank)}</span>${chevron}
    </div>
    <a class="relative [grid-area:avatar]" href="${profileHref}" tabindex="-1"
      ><img
...
<p class="mt-[0.4rem] wrap-sep text-[0.68rem] text-dimmer">
    ...
    <div class="flex flex-col items-end text-right [grid-area:score]">
      <span
        class="font-display text-2xl leading-none font-extrabold tracking-[-0.03em] tabular-nums max-phone:text-[1.2rem] ${leadsTotal
          ? "text-accent"
          : ""}"
        >${formatNumber(user.totalContributions)}${leadsTotal
          ? html`<span class="sr-only"> — highest total on the board</span>`
          : null}</span
...
export function userRowHtml(options: UserRowOptions): Html {
  ...
  if (!goals) {
    return html`<article class="${ROW_CARD} ${border}" style="--i:${options.rank}">
      <div class="relative ${ROW_GRID}">${userRowBody(options, false)}</div>
    </article>`;
  }
  ...
  return html`<details class="${ROW_CARD} ${border}" style="--i:${options.rank}">
    <summary
      class="relative list-none cursor-pointer [&::-webkit-details-marker]:hidden focus-visible:rounded-2xl ${ROW_GRID}"
    >
      ${userRowBody(options, true)}
    </summary>
    <div class="overflow-hidden rounded-b-[15px]">${userGoalsBandHtml(goals)}</div>
  </details>`;
}
...
export function allTimeRowHtml(options: AllTimeRowOptions): Html {
  ...
  const score = best
    ? html`<span
        class="mt-[0.4rem] text-[0.68rem] ${leadsYear ? "text-accent" : "text-dim"}"
        >best ${formatNumber(best.count)} in ${best.year}${leadsYear
          ? html`<span class="sr-only"> — highest single year on the board</span>`
          : null}</span
      >`
    : html`<span class="mt-[0.4rem] text-[0.68rem] text-dim">no activity</span>`;

  return html`<article
    class="${ROW_CARD} ${lead ? "border-accent/32" : "border-line-soft"} ${ROW_GRID}"
    style="--i:${rank}"
  >
    <div
      class="text-[0.78rem] tracking-[0.06em] [grid-area:rank] ${lead
        ? "text-accent"
        : "text-dimmer"}"
      aria-hidden="true"
    >
    ...
    <a class="[grid-area:avatar]" href="${profileHref}" tabindex="-1"
      ><img
        class="size-[52px] rounded-[13px] border border-line bg-heat-0 saturate-[0.85] transition-[filter,border-color] duration-200 group-hover:border-accent/40 group-hover:saturate-100 max-phone:size-[42px] max-phone:rounded-[11px]"
        src="${user.avatarUrl}"
        alt=""
    ...
    <div class="min-w-0 [grid-area:plot]">${yearStripSvg(cells, {
    cell: 34,
    gap: 5,
    podium: true,
    ...
    <div class="flex flex-col items-end text-right [grid-area:score]">
      <span
        class="font-display text-2xl leading-none font-extrabold tracking-[-0.03em] tabular-nums max-phone:text-[1.2rem] ${leadsTotal
          ? "text-accent"
          : ""}"
        >${formatNumber(user.total)}${leadsTotal
          ? html`<span class="sr-only"> — highest total on the board</span>`
          : null}</span
      ><span class="mt-[0.3rem] text-[0.62rem] tracking-[0.12em] text-dimmer uppercase"
        >contributions</span
